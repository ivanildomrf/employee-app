(function () {
    ...
    AuthenticationService.$inject = ['$http', '$location', '$cookies', '$rootScope'];
    function AuthenticationService($http, $location, $cookies, $rootScope) {
        var service = {};
        ...
        function Login(email, password, callback) {
            ...
            var url = GetBaseURL() + '/api/authenticate';
        ...
        function GetUserByUsername(id, baseURL, callback) {
            ...
            return $http.get(url+id).then(function (response) {
                ...
                if (response.data && response.status === 200) {
                    resp = { success: true, id: response.data.id, firstName: response.data.firstName, lastName: response.data.lastName, username: response.data.username };
                } else if (response.data && response.status === 401)  {
                    resp = { success: false, message: "Credenciais inválidas, tente novamente!"}; 
                } else {
                    resp = { success: false, message: "Erro inesperado, favor tente novamente em alguns minutos"}; 
                ...
                if (response.message !== null && response.status === 401) {
                    message = "Credenciais informadas são inválidas, acesso não autorizado";
                } else {
                    message = "Erro inesperado, favor entrar em contato com o administrador do sistema";
                }
